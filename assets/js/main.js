...
(function ($) {
	...
	loadImageList()
		...
		.catch(function (error) {
			if (window.console) console.error("Unable to load images.json", error);
		});

	function loadImageList() {
		var imagesJsonUrl = $main.data("images-json-url");

		if (!imagesJsonUrl) return Promise.resolve([]);

		return fetch(imagesJsonUrl)
	...
	function getImageItem(item) {
		var imageBaseUrl = removeTrailingSlash($main.data("image-base-url") || ""),
			src,
			thumb,
			alt = "",
			position = "";
		...
		if (typeof item === "string") {
			...
			src = item.src || item.file || item.name;
		...
		return {
			src: toImageUrl(src, imageBaseUrl),
			thumb: toImageUrl(thumb, imageBaseUrl),
			alt: alt,
			name: src || "",
			position: position
		};
	...
	function toImageUrl(value, baseUrl) {
		if (!value) return "";
